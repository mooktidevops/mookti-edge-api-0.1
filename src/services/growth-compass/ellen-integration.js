...
exports.growthCompassIntegration = exports.GrowthCompassEllenIntegration = void 0;
const kv_1 = require("@vercel/kv");
/**
 * Service to integrate Ellen sessions with Growth Compass tracking
 */
class GrowthCompassEllenIntegration {
    COMPASS_PREFIX = 'growth:compass:';
    METRICS_PREFIX = 'growth:metrics:';
    PATTERN_PREFIX = 'growth:patterns:';
    /**
     * Update Growth Compass based on Ellen session activity
     */
    async updateFromSession(update) {
     ...
     */
    calculateContributions(metrics, session) {
        return {
    ...
    calculateGoalAlignment(session) {
        // Check if session had clear goals and if they were met
        const hasGoal = session.sessionGoal && session.sessionGoal.length > 0;
        const goalsAddressed = session.messages.filter(m => m.role === 'assistant' &&
            m.content.toLowerCase().includes('goal') ||
            m.content.toLowerCase().includes('objective')).length;
        if (!hasGoal)
            return 0.3; // Unfocused session
        if (goalsAddressed === 0)
            return 0.5; // Had goal but didn't address
        return Math.min(0.7 + (goalsAddressed * 0.1), 1); // Goal-focused work
    }
    calculateBalance(toolsUsed) {
    ...
    calculateRecovery(metrics) {
        if (metrics.strugglesIdentified === 0)
            return 0.5; // No struggles = no recovery needed
     ...
     */
    updateComponents(compass, contributions) {
     ...
     */
    async detectPatterns(userId, metrics) {
        const patterns = [];
        // Deep Diver pattern
    ...
    async storeCompass(compass) {
        try {
            const key = `${this.COMPASS_PREFIX}${compass.userId}`;
            await kv_1.kv.set(key, compass, {
    ...
    async storeSessionMetrics(userId, metrics) {
        try {
            const key = `${this.METRICS_PREFIX}${userId}:${metrics.sessionId}`;
            await kv_1.kv.set(key, metrics, {
    ...
    async storePatterns(userId, patterns) {
        try {
            ...
            await kv_1.kv.set(key, Array.from(patternMap.values()), {
                ex: 60 * 60 * 24 * 90 // 90 days TTL
            });
        }
        catch (error) {
    ...
    async getHistory(userId, days = 30) {
        try {
            ...
            for (const key of keys) {
                const metric = await kv_1.kv.get(key);
...
exports.GrowthCompassEllenIntegration = GrowthCompassEllenIntegration;
// Export singleton
exports.growthCompassIntegration = new GrowthCompassEllenIntegration();
