...
export interface CoachingContext {
  content?: string; // The text to review/improve
  purpose?: string; // What the content is for
  audience?: string; // Who will read/receive it
  constraints?: string[]; // Word limits, format requirements, etc.
  priorFeedback?: string[]; // Previous suggestions
  userMessage: string;
...
export interface CoachingResponse {
  feedback: string;
  suggestions?: string[];
  revised?: string; // Revised version if applicable
  metadata?: {
    toolName: string;
    focusAreas?: string[];
 ...
 */
export class WritingCoachTool {
  async execute(context: CoachingContext): Promise<CoachingResponse> {
...
- Provide more than 3-4 main suggestions`;
  ...
  private buildWritingPrompt(context: CoachingContext): string {
  ...
  private parseWritingResponse(text: string): CoachingResponse {
 ...
 */
export class NoteAssistantTool {
  async execute(context: CoachingContext): Promise<CoachingResponse> {
 ...
 */
export class OfficeHoursCoachTool {
  async execute(context: CoachingContext): Promise<CoachingResponse> {
...
Help prepare for a productive office hours visit.`;
  ...
  private generateQuestionSuggestions(context: CoachingContext): string[] {
...
Provide email coaching and improvements.`;
  ...
  private async generateRevisedEmail(context: CoachingContext): Promise<string> {
    const { text } = await generateText({
      model: context.modelRouting.model,
 ...
 */
export class CoachingToolsOrchestrator {
  private writingCoach = new WritingCoachTool();
  private noteAssistant = new NoteAssistantTool();
  private officeHoursCoach = new OfficeHoursCoachTool();
  private emailCoach = new EmailCoachTool();
  ...
  private selectBestTool(context: CoachingContext): string {
