...
export interface EvaluatorParams {
  ...
  criteria?: string[];
  context?: any;
  decisionType?: 'academic' | 'course_selection' | 'study_strategy' | 'time_management' | 'general';
}
 ...
 */
export class EvaluatorTool {
  name = 'evaluator_tool';
  description = 'Helps evaluate options and make informed academic decisions';
  
  async execute(params: EvaluatorParams): Promise<EvaluatorResult> {
...
export const evaluatorTool = new EvaluatorTool();