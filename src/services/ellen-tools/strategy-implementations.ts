...
export interface StrategyContext {
  content?: string; // Material to work with
  subject?: string;
...
interface StrategyStep {
  ...
  checkpoint?: string; // How to know if done correctly
}

/**
 * Class Troubleshooter - Diagnose and fix specific class problems
 */
export class ClassTroubleshooterTool {
  async execute(context: StrategyContext): Promise<StrategyOutput> {
...
Diagnose the specific issue and provide targeted solution steps.`;
  ...
  private parseTroubleshooterResponse(text: string, context: StrategyContext): StrategyOutput {
 ...
 */
export class RetrievalPracticeTool {
  async execute(context: StrategyContext): Promise<StrategyOutput> {
...
Create specific retrieval practice steps.`,
  ...
  private generateRetrievalSteps(context: StrategyContext): StrategyStep[] {
    return [
      {
        step: 1,
 ...
 */
export class SelfExplanationTool {
  async execute(context: StrategyContext): Promise<StrategyOutput> {
 ...
 */
export class DesirableDifficultiesPracticeTool {
  async execute(context: StrategyContext): Promise<StrategyOutput> {
 ...
 */
export class StrategyImplementationOrchestrator {
  private classTroubleshooter = new ClassTroubleshooterTool();
  private retrievalPractice = new RetrievalPracticeTool();
  private selfExplanation = new SelfExplanationTool();
  private dualCoding = new DualCodingTool();
  private desirableDifficulties = new DesirableDifficultiesPracticeTool();
  ...
  private async executeStrategy(name: string, context: StrategyContext): Promise<StrategyOutput> {
    switch (name) {
      case 'troubleshooter':
        return this.classTroubleshooter.execute(context);
      case 'retrieval':
