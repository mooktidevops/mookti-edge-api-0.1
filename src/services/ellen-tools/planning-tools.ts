...
export interface PlanningContext {
  goal?: string;
  timeframe?: string; // "1 week", "1 month", "semester", etc.
  subjects?: string[];
  constraints?: string[]; // "work 20hrs/week", "3 exams next month"
  currentStrategies?: string[];
  preferredTimes?: string[]; // "mornings", "evenings"
  sessionType?: 'study' | 'writing' | 'review' | 'practice';
  userMessage: string;
...
export interface StudyPlan {
  ...
  strategies: string[];
  woopPlan?: WOOPStructure;
  metadata?: {
    toolName: string;
    totalHours?: number;
    techniqueRecommendations?: string[];
    difficultyCalibration?: 'aggressive' | 'balanced' | 'gentle';
  };
}
...
interface ScheduleBlock {
  ...
  subject: string;
  technique: string; // "active recall", "practice problems", etc.
...
interface WOOPStructure {
  wish: string;
  outcome: string;
  obstacles: string[];
  plan: string[]; // If-then statements
}

/**
 * Plan Manager - Study planning with spaced repetition and interleaving
 */
export class PlanManagerTool {
  async execute(context: PlanningContext): Promise<StudyPlan> {
...
4. Implementation intentions for obstacles`;
  ...
  private parsePlanResponse(text: string, context: PlanningContext): StudyPlan {
  ...
  private extractWOOP(text: string): WOOPStructure {
  ...
  private extractScheduleBlocks(text: string): ScheduleBlock[] {
  ...
  private generateMilestones(context: PlanningContext): Milestone[] {
...
Design an optimal focus session structure.`;
  ...
  private parseFocusResponse(text: string, context: PlanningContext): StudyPlan {
  ...
  private extractSessionStructure(text: string): ScheduleBlock[] {
    const blocks: ScheduleBlock[] = [];
  ...
  private calculateTotalHours(blocks: ScheduleBlock[]): number {
    return blocks.reduce((total, block) => total + block.duration, 0) / 60;
  }

  private getSessionTechniques(context: PlanningContext): string[] {
 ...
 */
export class PlanningToolsOrchestrator {
  private planManager = new PlanManagerTool();
  private focusSession = new FocusSessionTool();
