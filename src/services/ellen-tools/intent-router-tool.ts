...
import type { LearningIntent, EngagementDepth } from '../user-state-monitor';

export interface IntentAnalysis {
  primaryIntent: LearningIntent;
  secondaryIntent?: LearningIntent;
...
export class IntentRouterTool {
  private toolMatrix: Record<LearningIntent, Record<EngagementDepth, string>> = {
    understand: {
  ...
  private fallbackTools: Record<string, string> = {
  ...
  async execute(context: RouterContext): Promise<IntentAnalysis> {
  ...
  private async analyzeIntent(context: RouterContext): Promise<IntentAnalysis> {
...
- create: Longer-form writing, artistic processes, drafting, composing
...
ENGAGEMENT DEPTHS:
- surface: Quick facts, definitions, and straightforward answers for simple, single-step questions with objective solutions
- guided: Step-by-step help and structured support for learning more complex topics including subjective questions
- deep: Thorough exploration, philosophical discussion of the most difficult questions (both subjective and objective)
...
Return JSON with this structure:
{
  ...
  private routeToTools(intent: IntentAnalysis): string[] {
    const tools: string[] = [];
  ...
  private getDefaultTool(intent: IntentAnalysis): string {
    const tool = this.toolMatrix[intent.primaryIntent]?.[intent.depth] || 'socratic_tool';
    return this.fallbackTools[tool] || tool;
  }

  private generateClarificationPrompt(intent: IntentAnalysis): string {
    const depthOptions = {
  ...
  areToolsAvailable(tools: string[]): boolean {
...
export const intentRouter = new IntentRouterTool();