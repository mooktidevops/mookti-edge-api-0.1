...
// Define types locally since we're creating a new tool category
export interface EllenToolResult {
  content: string;
  metadata?: {
    toolSwitchSuggested?: boolean;
...
export interface EllenTool {
  name: string;
  description: string;
  execute: (params: any) => Promise<EllenToolResult>;
}
 ...
 */
export const frustrationHandler: EllenTool = {
 ...
 */
export const toolSwitchHandler: EllenTool = {
  name: 'tool_switch',
 ...
 */
export const expectationClarifier: EllenTool = {
 ...
 */
export const feedbackCollector: EllenTool = {
 ...
 */
function quickFrustrationCheck(message: string): {
  isObvious: boolean;
  result: any;
} {
 ...
 */
function detectFrustrationKeywords(message: string): boolean {
 ...
 */
function detectToolSwitchKeywords(message: string): boolean {
...
export async function analyzeEngagementPattern(
  ...
  pattern: 'active' | 'declining' | 'minimal' | 'frustrated';
  ...
  const hasRepetition = lastThree.some((msg, i) => 
    lastThree.slice(i + 1).some(other => 
      similarityScore(msg, other) > 0.7
    )
  );
