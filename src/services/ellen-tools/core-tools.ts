...
import { ResearchFoundations, CognitiveLoadPrinciples, EmotionalSupportPrinciples } from './research-foundations';
...
export class SocraticTool {
  private readonly foundation = ResearchFoundations.socratic;
  
  async execute(context: ToolContext): Promise<ToolResponse> {
  ...
  private buildSystemPrompt(context: ToolContext): string {
    ...
    const emotionalGuidance = hasEmotionalSignals 
      ? `\nEMOTIONAL AWARENESS: User shows ${context.emotionalSignals?.join(', ')}. Be especially ${
          context.emotionalSignals?.includes('frustrated') ? 'encouraging and break down complexity' :
...
- Withhold direct answers to specific questions`;
  ...
  private buildUserPrompt(context: ToolContext): string {
  ...
  private assessCognitiveLoad(response: string): 'low' | 'medium' | 'high' {
    const questionCount = (response.match(/\?/g) || []).length;
    const conceptCount = (response.match(/\b(principle|concept|idea|theory|framework)\b/gi) || []).length;
    
    if (questionCount > 2 || conceptCount > 3) return 'high';
    if (questionCount > 1 || conceptCount > 1) return 'medium';
    return 'low';
  }

  private generateEmotionalSupport(context: ToolContext): string | undefined {
    if (!context.emotionalSignals || context.emotionalSignals.length === 0) return undefined;
...
export class ReflectionTool {
  private readonly foundation = ResearchFoundations.reflection;
...
export class ExtensionTool {
  private readonly foundation = ResearchFoundations.extension;
...
- CREATE: Generate novel combinations`;
  ...
  private buildExtensionPrompt(context: ToolContext): string {
    let prompt = `User says: "${context.userMessage}"`;
 ...
 */
export class GenealogyTool {
  private readonly foundation = ResearchFoundations.genealogy;
 ...
 */
export class EllenToolOrchestrator {
  private socraticTool = new SocraticTool();
  private reflectionTool = new ReflectionTool();
  private extensionTool = new ExtensionTool();
  private genealogyTool = new GenealogyTool();
  
  async selectAndExecute(context: ToolContext, toolName?: string): Promise<ToolResponse> {
  ...
  private selectBestTool(context: ToolContext): string {
  ...
  private async executeTool(toolName: string, context: ToolContext): Promise<ToolResponse> {
    switch (toolName) {
      case 'socratic':
   ...
   */
  assessCognitiveLoadBalance(responses: ToolResponse[]): {
