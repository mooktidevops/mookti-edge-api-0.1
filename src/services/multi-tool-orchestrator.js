...
exports.MultiToolOptimizer = exports.StateAwareMultiToolOrchestrator = void 0;
...
class StateAwareMultiToolOrchestrator {
    ...
    constructor(intentRouter, queryOptimizer, contextManager, ellenTools) {
        this.intentRouter = intentRouter;
        this.queryOptimizer = queryOptimizer;
        this.contextManager = contextManager;
        this.ellenTools = ellenTools;
    }
    async orchestrate(query, userState, previousState, sessionId) {
    ...
    async determinePattern(currentState, previousState, query) {
        ...
        if (previousState && currentState.intent.changed) {
            ...
            return {
                ...
                reason: changeReason,
                tools: [
                    previousState.tooling.currentToolAppropriate || 'socratic_tool',
        ...
        if (previousState && currentState.depth.changeIndicator) {
            const tools = this.getDepthProgressionTools(currentState.intent.current, previousState.depth.current, currentState.depth.current);
            return {
                type: 'chain',
        ...
        if (intents.length > 1) {
            const tools = intents.map(intent => this.intentRouter.selectToolForIntent(intent, currentState.depth.current));
            return {
                type: 'parallel',
        ...
        // 4. Fallback Pattern: High frustration or tool ineffectiveness
        if (currentState.sentiment.frustrationLevel > 0.7) {
            return {
                type: 'fallback',
                ...
                context: {
                    frustrationLevel: currentState.sentiment.frustrationLevel,
    ...
    async executeHandoff(pattern, query, userState, sessionId) {
        const executions = [];
        for (let i = 0; i < pattern.tools.length; i++) {
            ...
            const result = await this.executeTool(toolName, query, userState, context, sessionId);
    ...
    async executeParallel(pattern, query, userState, sessionId) {
        // Execute all tools in parallel
        const executionPromises = pattern.tools.map(toolName => this.executeTool(toolName, query, userState, pattern.context, sessionId));
    ...
    async executeChain(pattern, query, userState, sessionId) {
        const executions = [];
        let currentQuery = query;
        let accumulatedContext = { ...pattern.context };
        for (const toolName of pattern.tools) {
            const result = await this.executeTool(toolName, currentQuery, userState, accumulatedContext, sessionId);
            executions.push(result);
    ...
    async executeFallback(pattern, query, userState, sessionId) {
        const executions = [];
        for (const toolName of pattern.tools) {
            const result = await this.executeTool(toolName, query, userState, pattern.context, sessionId);
            executions.push(result);
    ...
    async executeSingle(toolName, query, userState, sessionId) {
        const result = await this.executeTool(toolName, query, userState, {}, sessionId);
        return [result];
    }
    async executeTool(toolName, query, userState, context, sessionId) {
        const startTime = Date.now();
        try {
    ...
    async detectMultipleIntents(query) {
...
Return format: intent1,intent2,intent3`;
    ...
    getDepthProgressionTools(intent, fromDepth, toDepth) {
    ...
    extractNextQuery(previousResult, originalQuery) {
    ...
    calculatePatternEffectiveness(executions, userState) {
        let score = 0;
        const weights = {
            success: 0.4,
            speed: 0.2,
        ...
        const appropriateTools = executions.filter(e => e.tool === userState.tooling.suggestedTool ||
            (typeof userState.tooling.currentToolAppropriate === 'string' && e.tool === userState.tooling.currentToolAppropriate)).length / executions.length;
...
exports.StateAwareMultiToolOrchestrator = StateAwareMultiToolOrchestrator;
// Multi-Tool Optimizer for pre-warming and optimization
class MultiToolOptimizer {
    preWarmCache = new Map();
    patternHistory = new Map();
    async preWarmTools(predictedTools, context) {
    ...
    predictNextTools(sessionId, currentState, recentPatterns) {
...
exports.MultiToolOptimizer = MultiToolOptimizer;
