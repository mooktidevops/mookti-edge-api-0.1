...
exports.FormativeController = void 0;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
class FormativeController {
    anthropic;
    constructor() {
    ...
    async process(toolName, payload, modelTier) {
        try {
            let response;
            switch (toolName) {
    ...
    async handleFormativeCheck(request, modelTier) {
...
Provide formative feedback following the structure.`;
    ...
    parseFormativeCheckResponse(text) {
    ...
    async handleDiagnosticProbe(request, modelTier) {
...
Level: ${request.level || 'intro'}`;
    ...
    parseDiagnosticProbeResponse(text, request) {
    ...
    async handleRevisionScheduler(request, modelTier) {
    ...
    selectTopicsForDay(topics, day, totalDays) {
    ...
    generateInterleavingPattern(topics) {
    ...
    async handleWorkedExample(request, modelTier) {
...
${request.highlight_strategy ? 'Highlight the overall strategy.' : ''}`;
    ...
    parseWorkedExampleResponse(text, request) {
        const steps = [];
        const lines = text.split('\n');
        let currentStep = '';
        let currentWhy = '';
    ...
    async handleConceptMapper(request, modelTier) {
    ...
    generateRelatedConcepts(concept, domain) {
    ...
    generatePrerequisites(concept) {
        return [`Basic ${concept.split(' ')[0]}`, 'Foundational principles'];
    }
    generateApplications(concept) {
...
exports.FormativeController = FormativeController;
