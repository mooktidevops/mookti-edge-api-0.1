...
exports.SocraticController = void 0;
...
class SocraticController {
    ...
    async process(request, modelTier) {
        try {
    ...
    async generateSocraticResponse(request, modelTier) {
...
GENTLE CHALLENGE: ${request.policy.gentle_challenge}`;
    ...
    parseModelResponse(text, request) {
    ...
    validateSocraticResponse(response) {
        ...
        if (response.extension_question)
            questionCount++;
        if (response.assumption_probe)
            questionCount++;
        if (questionCount > 2) {
...
exports.SocraticController = SocraticController;
