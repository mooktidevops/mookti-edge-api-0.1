...
export type ContextStrategy = 
  ...
  | 'minimal'  // Current exchange only (last 2 messages)
  | 'recent'   // Last 3 exchanges (6 messages)
  | 'full'     // Complete history (with compression if needed)
  | 'summary'  // Compressed older + recent
  | 'goals'    // Just objectives/constraints
  | 'document'; // For writing tutor - the document being edited
...
export interface ToolConfig {
  ...
  modelTier: ModelTier;
  category: 'pedagogical' | 'coaching' | 'planning' | 'diagnostic' | 'utility';
  description?: string;
...
export function getToolConfig(toolName: string): ToolConfig {
  return TOOL_CONFIG[toolName] || TOOL_CONFIG.socratic_tool;
}

export function toolNeedsRetrieval(toolName: string): boolean {
  return getToolConfig(toolName).needsRetrieval;
}

export function getToolModelTier(toolName: string): ModelTier {
  return getToolConfig(toolName).modelTier;
}

export function getToolContextStrategy(toolName: string): ContextStrategy {
