...
import {
  ...
  SessionMessageAddRequest,
  SessionMessage,
  SessionStatus,
  ProcessMetrics
} from './ellen-session-types';

export class EllenSessionStorage {
  private readonly SESSION_PREFIX = 'ellen:session:';
  private readonly USER_SESSIONS_PREFIX = 'ellen:user:sessions:';
  ...
  async createSession(request: SessionCreateRequest): Promise<EllenSession> {
    ...
    await kv.sadd(`${this.USER_SESSIONS_PREFIX}${request.userId}`, sessionId);

    return session;
  }

  // Get a session by ID
  async getSession(sessionId: string): Promise<EllenSession | null> {
    const data = await kv.get(`${this.SESSION_PREFIX}${sessionId}`);
    if (!data) return null;
  ...
  async updateSession(
    ...
    const updatedSession: EllenSession = {
      ...session,
      ...updates,
      lastActiveAt: now,
  ...
  async addMessage(request: SessionMessageAddRequest): Promise<SessionMessage | null> {
    ...
    // Store updated session (Vercel KV auto-serializes objects)
    await kv.setex(
      `${this.SESSION_PREFIX}${request.sessionId}`,
      this.SESSION_TTL,
      session
    );
  ...
  async getUserSessions(
    ...
    const sessionIds = await kv.smembers(`${this.USER_SESSIONS_PREFIX}${userId}`);
    ...
    for (const sessionId of sessionIds) {
      ...
      if (filters) {
        ...
        if (filters.startDate && session.startedAt < filters.startDate) continue;
        if (filters.endDate && session.startedAt > filters.endDate) continue;
        if (filters.minDuration && session.totalDuration < filters.minDuration) continue;
      }
  ...
  // Resume a session
  async resumeSession(sessionId: string): Promise<EllenSession | null> {
  ...
  async deleteSession(sessionId: string): Promise<boolean> {
    ...
    await kv.srem(`${this.USER_SESSIONS_PREFIX}${session.userId}`, sessionId);
  ...
  private updateProcessMetrics(metrics: ProcessMetrics, message: SessionMessage): void {
    if (!message.metadata?.processType) return;
  ...
  private calculateCompletionRate(session: EllenSession): number {
    if (!session.sessionGoal) return 0;
  ...
  private calculateGrowthContributions(session: EllenSession): any {
    const contributions: any = {};

    // Goal Alignment: Based on completion and duration
    if (session.status === 'completed') {
      contributions.goalAlignment = Math.min(100, 
        (session.totalDuration / 30) * 50 + // 30 min = 50 points
        (session.keyTakeaways?.length || 0) * 10 // Each takeaway = 10 points
      );
    }

    // Process Engagement: Based on process metrics
    contributions.processEngagement = Math.min(100,
      (session.processMetrics.retrievalAttempts || 0) * 15 +
    ...
    contributions.depthScore = Math.min(100,
      Math.min(session.totalDuration / 45, 1) * 50 + // 45 min = full duration score
      (session.understandingRating || 0) * 10 // Understanding rating contribution
    );

    // Reflection Quality: Based on takeaways and ratings
    if (session.keyTakeaways && session.keyTakeaways.length > 0) {
      contributions.reflectionQuality = Math.min(100,
        session.keyTakeaways.length * 20 +
