...
exports.DATA_THRESHOLDS = void 0;
exports.checkDataSufficiency = checkDataSufficiency;
exports.getDataSufficiencyMessages = getDataSufficiencyMessages;
exports.shouldShowFeature = shouldShowFeature;
// Minimum thresholds before making recommendations
exports.DATA_THRESHOLDS = {
 ...
 */
function checkDataSufficiency(weeksActive, totalSessions, sessionsPerWindow, daysActive) {
    ...
    const rhythmReady = totalSessions >= exports.DATA_THRESHOLDS.rhythm.minSessions &&
        weeksActive >= exports.DATA_THRESHOLDS.rhythm.minWeeks &&
        Array.from(sessionsPerWindow.values()).some(count => count >= exports.DATA_THRESHOLDS.rhythm.minSessionsPerWindow);
    if (!rhythmReady) {
        missingDataTypes.push('rhythm_patterns');
    }
    // Check power patterns
    const powerPatternsReady = weeksActive >= exports.DATA_THRESHOLDS.powerPatterns.minWeeks &&
        totalSessions >= exports.DATA_THRESHOLDS.powerPatterns.minSessionsBeforeExperiment;
    if (!powerPatternsReady) {
        missingDataTypes.push('power_patterns');
    }
    // Check sync score
    const syncScoreReady = totalSessions >= exports.DATA_THRESHOLDS.syncScore.minSessions &&
        daysActive >= exports.DATA_THRESHOLDS.syncScore.minDaysActive;
    if (!syncScoreReady) {
        missingDataTypes.push('sync_score');
    }
    // Check season suggestions
    const seasonSuggestionsReady = weeksActive >= exports.DATA_THRESHOLDS.seasons.minWeeks &&
        totalSessions >= exports.DATA_THRESHOLDS.seasons.minSessionsForSuggestion;
 ...
 */
function calculateConfidence(weeksActive, totalSessions, daysActive) {
 ...
 */
function getDataSufficiencyMessages(sufficiency) {
    const messages = {
        primary: '',
        details: [],
        recommendations: []
    ...
    switch (sufficiency.timeline.phase) {
        case 'collecting':
            ...
            messages.details.push(`${sufficiency.sessionsLogged || 0} sessions logged so far`);
 ...
 */
function shouldShowFeature(feature, sufficiency) {
    switch (feature) {
        case 'primeSlots':
