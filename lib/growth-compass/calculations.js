...
exports.calculateGoalAlignment = calculateGoalAlignment;
exports.calculateBalanceIndex = calculateBalanceIndex;
exports.calculateDepthScore = calculateDepthScore;
exports.calculateRecoveryEngagement = calculateRecoveryEngagement;
exports.calculateReflectionQuality = calculateReflectionQuality;
exports.calculateSyncScore = calculateSyncScore;
exports.identifyOptimalWindows = identifyOptimalWindows;
...
function calculateGrowthVelocity(components, season = 'steady') {
    const weights = SEASON_WEIGHTS[season];
    return Math.round(components.goalAlignment * weights.goalAlignment +
...
function calculateGoalAlignment(completedTasks, plannedTasks, actualTime, plannedTime) {
    if (plannedTasks === 0)
        return 50; // Baseline when no tasks planned
...
function calculateBalanceIndex(actual, target) {
...
function calculateDepthScore(focusMinutes, taskComplexity, // 1-5 scale
understandingRatings // 1-5 scale
) {
    if (focusMinutes.length === 0)
        return 50; // Baseline
    ...
    return Math.round(focusScore * 0.4 +
        avgComplexity * 0.3 +
        avgUnderstanding * 0.3);
}
function calculateRecoveryEngagement(activities, targetPerWeek = 7) {
    // Count activities in last 14 days
    const twoWeeksAgo = new Date();
    twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);
    const recentActivities = activities.filter(a => new Date(a.timestamp) >= twoWeeksAgo);
...
function calculateReflectionQuality(reflectionTexts, onDevice = true) {
    ...
    if (onDevice) {
        ...
        for (const text of reflectionTexts) {
            ...
            if (text.length >= 50 && text.length <= 200)
                ...
                score += 10;
            // Depth markers
            if (text.includes('because') || text.includes('realize'))
                score += 10;
            if (text.includes('next') || text.includes('will'))
                score += 10;
            if (text.includes('?'))
                score += 5; // Questions show thinking
...
function calculateSyncScore(rhythmData, actualSessions, optimalWindows) {
    if (actualSessions.length < 5)
        return 50; // Baseline
    ...
    for (const session of actualSessions) {
        ...
        const isAligned = optimalWindows.some(window => window.dayOfWeek === sessionDay &&
...
function identifyOptimalWindows(heatmapData, minSessions = 3, performanceThreshold = 70) {
    const windows = [];
    for (let day = 0; day < 7; day++) {
        ...
        for (const cell of dayData) {
            if (cell.sessionCount >= minSessions && cell.performance >= performanceThreshold) {
                ...
                const existingWindow = windows.find(w => w.dayOfWeek === day &&
