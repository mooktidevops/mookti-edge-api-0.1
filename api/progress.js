...
function calculateCompletionPercentage(completedNodes, totalNodes) {
    return Math.round((completedNodes.length / totalNodes) * 100);
}
function resolveConflicts(local, remote) {
    const conflicts = [];
    if (!remote) {
    ...
    // Merge time spent
    const resolvedTimeSpent = Math.max(local.total_time_spent || 0, remote.total_time_spent);
    return {
        ...
        resolved_progress: {
            ...
            completion_percentage: calculateCompletionPercentage(Array.from(completedNodesSet), 200),
            session_id: local.session_id,
    ...
    try {
        ...
        if (req.method === 'GET' && pathSegments.length === 4) {
            ...
            if (requestedUserId !== userId) {
                return new Response(JSON.stringify({ error: 'Unauthorized', code: 'UNAUTHORIZED' }), { status: 403, headers: { 'Content-Type': 'application/json' } });
            }
        ...
        return new Response(JSON.stringify({ error: 'Invalid route', code: 'INVALID_ROUTE' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    }
    catch (error) {
