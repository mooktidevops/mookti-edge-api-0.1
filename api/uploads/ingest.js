...
async function handler(req) {
    ...
    try {
        ...
        if (!token) {
            return new Response(JSON.stringify({ error: 'Missing upload token' }), { status: 401, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        const { userId, docId, fileName, fileType } = tokenData;
        ...
        if (!content || content.trim().length === 0) {
            return new Response(JSON.stringify({ error: 'Empty file content' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
        }
        ...
        // Log usage
        await manager_1.EntitlementsManager.getInstance().logUsage(userId, 'upload', 1);
        // Delete the upload token
        await kv.del(`upload_token_${token}`);
        // Store document metadata
        await kv.set(`doc_${docId}`, {
            id: docId,
            userId,
            fileName,
            fileType,
