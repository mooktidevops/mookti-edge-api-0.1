...
const entitlements_1 = require("../lib/ai/entitlements");
...
const buildEllenPrompt = (nodeId, progress) => {
    let progressContext = '';
    if (nodeId || progress) {
...
async function handler(req) {
    ...
    try {
        ...
        if (!allowed) {
            return new Response(JSON.stringify({ error: 'Too Many Requests', code: 'RATE_LIMIT' }), { status: 429, headers: { 'Content-Type': 'application/json' } });
        }
        // Verify auth (dev mode bypass supported)
        let userId;
        if ((0, dev_mode_1.isDevMode)(req)) {
            const dev = (0, dev_mode_1.getDevUser)(req);
            userId = dev?.id || 'dev-user-default';
        }
        else {
            const authResult = await (0, auth_middleware_1.verifyApiAuth)(req);
        ...
        const userTier = 'pro'; // TODO: Get from user profile
        const entitlements = (0, entitlements_1.getUserEntitlement)(userTier);
        // Route to appropriate model based on user selection and entitlements
        const modelRequest = {
            provider: body.provider,
            modelId: body.modelId,
            tier: 2, // Default to tier 2 (balanced)
        };
        let modelRouting;
        try {
            modelRouting = (0, model_router_1.routeToModel)(modelRequest);
        }
        catch (error) {
