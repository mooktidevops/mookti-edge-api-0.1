...
async function testEndpoint(name, method, path, body, headers) {
...
async function runTests() {
    ...
    const testSuites = [
        {
            ...
            tests: async () => {
                ...
                await testEndpoint('Search Test Endpoint', 'POST', '/search-test', {
        ...
        },
        {
            ...
            tests: async () => {
                ...
                await testEndpoint('Claude API (No Auth)', 'POST', '/claude', { prompt: 'Hello' }, { Authorization: 'Bearer invalid-token' });
                await testEndpoint('Search API (No Auth)', 'POST', '/search', { query: 'test query' }, { Authorization: 'Bearer invalid-token' });
            },
        },
        {
            ...
            tests: async () => {
                ...
                await testEndpoint('Get Learning Paths', 'GET', '/learning-paths');
                await testEndpoint('Get Specific Learning Path', 'GET', '/learning-paths/workplace_success');
                await testEndpoint('Get Learning Path with Content', 'GET', '/learning-paths/workplace_success?include_content=true');
                await testEndpoint('Get Node Context', 'GET', '/learning-paths/workplace_success/nodes/1/context?radius=2');
            },
        },
        {
            name: '4️⃣ Progress Tracking',
            tests: async () => {
                await testEndpoint('Get Progress (No Auth)', 'GET', '/progress/workplace_success', undefined, { Authorization: 'Bearer invalid-token' });
                await testEndpoint('Sync Progress (No Auth)', 'POST', '/progress/sync', {
        ...
        },
        {
            ...
            tests: async () => {
                await testEndpoint('Get Signed URL (No Auth)', 'POST', '/uploads/signed-url', {
                    filename: 'test.pdf',
                    contentType: 'application/pdf',
                    size: 1024,
                }, { Authorization: 'Bearer invalid-token' });
                await testEndpoint('Delete Document (No Auth)', 'DELETE', '/uploads/delete', { docId: 'test-doc-id' }, { Authorization: 'Bearer invalid-token' });
            },
        },
        {
            name: '6️⃣ Monitoring Endpoints',
            tests: async () => {
                await testEndpoint('Redis Stats', 'GET', '/monitoring/redis-stats', undefined, { Authorization: 'Bearer beta-monitoring-2024' });
                await testEndpoint('Monitor Dashboard', 'GET', '/../monitor.html');
            },
        },
        {
            name: '7️⃣ Chat/AI Endpoints',
            tests: async () => {
                await testEndpoint('Chat Endpoint (No Auth)', 'POST', '/chat', {
                    message: 'Hello Ellen',
                    chatHistory: [],
                    currentNodeId: '1',
                    useRAG: true,
    ...
    const authFailuresExpected = testResults.filter(r => r.status === 'fail' && r.statusCode === 401).length > 0;
    if (searchTestPassed) {
